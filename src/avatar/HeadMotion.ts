...
export class HeadMotion {
  ...
  update(delta: number, elapsed: number) {
    ...
    if (this.timer >= this.changeInterval) {
      ...
      // Up / down
      this.targetX = (Math.random() - 0.5) * 0.16;
    ...
    // Smooth movement
    const smoothing = Math.min(delta * 2, 1);

    this.currentX += (this.targetX - this.currentX) * smoothing;

    this.currentY += (this.targetY - this.currentY) * smoothing;

    this.currentZ += (this.targetZ - this.currentZ) * smoothing;

    // Very subtle idle movement
    const idleX = Math.sin(elapsed * 0.45) * 0.008;

    const idleY = Math.sin(elapsed * 0.35) * 0.012;

    const idleZ = Math.sin(elapsed * 0.25) * 0.006;

    this.target.rotation.x = this.currentX + idleX;

    this.target.rotation.y = this.currentY + idleY;

    this.target.rotation.z = this.currentZ + idleZ;
  }
