...
export class ProceduralBehavior {
  ...
  private updateBlink(delta: number) {
    ...
    if (
      !this.isBlinking &&
      this.blinkTimer >= this.nextBlinkTime
    ) {
  ...
  private updateGaze(delta: number, elapsed: number) {
    ...
    this.gazeX +=
      (this.targetGazeX - this.gazeX) * smoothing

    this.gazeY +=
      (this.targetGazeY - this.gazeY) * smoothing
    ...
    const finalX = THREE.MathUtils.clamp(
      this.gazeX + idleX,
      -1,
      1
    )
